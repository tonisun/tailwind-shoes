    ...
    <nav className="flex flex-wrap justify-between items-center">
        ...
        <div className={`${!isMobileMenuShown && "hidden"} w-full lg:block lg:w-auto`}>
            <ul className='lg:space-x-8 flex flex-col lg:flex-row p-4 cursor-pointer bg-gray-50 text-lg border lg:bg-transparent border-gray-100 rounded lg:border-none text-left'>
                {ROUTES.map((route, i) => {
                    return <li className={`rounded-lg py-2 px-3 ${i === 0 ?"bg-blue-500 text-white lg:bg-transparent lg:text-blue-500" : "hover:bg-gray-100"}`} key={route}>{route}</li>
                })}
            </ul>
        </div>
        ...
        <div className="fixed bottom-4 left-4 lg:static">
            <div className="flex-center h-12 w-12 rounded-full bg-white shadow-md p-2">
